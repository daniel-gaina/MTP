...
\subsection{Nominals}
 ...
 \begin{example} Specification of the attributes $\Read$ and $\del$:\tt
  ...
  eq-7 $\Forall{\Z}\at{\Z}\Read(\Empty)=\err$

  eq-8 $\Forall{\Ele ,\Lis} \at{\init}\Read(\Ele \comp \Lis) = \Ele$
 
  eq-9 $\Forall{\Ele,\Lis} \at{\init}\del( \Ele \comp \Lis) = \Lis$ 

  eq-10 $\Forall{\Z,\Ele,\Lis}\at{\Next(\Z)}\Read(\Ele \comp \Lis)= \at{\Z}\Read(\Lis)$

  eq-11 $\Forall{\Z,\Ele,\Lis}\at{\Next(\Z)}\del(\Ele \comp \Lis) = \Ele ; \del(\Z,\Lis)$ 
 \end{example}
